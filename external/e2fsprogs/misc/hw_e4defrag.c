...
#ifdef LOG_TAG
#undef LOG_TAG
#define LOG_TAG "e4defrag"
#endif
#define E4D_LOGE ALOGE
#else
#define E4D_LOGE printf
#endif
...
#define E4DEFRAG_MIN_SCORE 30
#define E4DEFRAG_MIN_SIZE 32*1024
#define E4DEFRAG_MAX_SIZE 100*1024*1024

static ext4_fsblk_t move_block_count = 0;
struct statvfs s;

void e4defrag_dmd_report(const char *str, float score, float defragment_score, ext4_fsblk_t blk_count, ext4_fsblk_t move_block_count)
{
	int dsm_fd;
	int buf_len;
	char *log_buf;
	...
	if (log_buf != NULL) {
		...
		buf_len = snprintf(log_buf, 1024, "dsm_e4defrag,22020,%s,%.0f,%.0f,%llu,%llu,%lu.\n", str, score, defragment_score, blk_count, move_block_count, (s.f_bfree * 100)/s.f_blocks);
		dsm_fd = open("/dev/dsm", O_WRONLY);
		if(-1 != dsm_fd) {
			write(dsm_fd, log_buf, buf_len);
			close(dsm_fd);
		}
		free(log_buf);
		log_buf = NULL;
...
static int is_ext4(const char *file, char *devname)
{
	...
	if (!fp) {
		E4D_LOGE("%s, open %s fail, errno %d, %s\n", __func__,
				MOUNT_PROCFS, errno, strerror(errno));
		return -1;
	}
	...
	for (;!feof(fp);) {
		...
		if (strncmp(dev, file_path, 5)) continue;
        //printf("Match.....\n");

		/* find the one */
		q = strlen(p) + p + 1;
		q = skip_whitespace(q);
		trancate_at_whitespace(q);

		if (0 == strcmp(q, FS_EXT4)) {
			strncpy(devname, buf, strlen(buf) + 1);
			fclose(fp);
			return 0;
		}
	}
		...
		int extents, ext4_fsblk_t blk_count)
{
	...
	if (ret < 0) {
		if ((mode_flag & DETAIL) && ret == -ENOSPC) {
			E4D_LOGE("\033[79;0H\033[K[%u/%u] \"%s\"\t\t"
				"  extents: %d -> %d\n", defraged_file_count,
			...
			struct FTW *ftwbuf EXT2FS_ATTR((unused)))
{
	...
	if (mode_flag & DETAIL) {
		if (total_count == 1 && regular_count == 1)
			...
			E4D_LOGE("[%u/%u]", defraged_file_count, total_count);
			fflush(stdout);
		}
	}
	...
	if (total_count == 1 && regular_count == 1) {
		...
		if (mode_flag & DETAIL) {
			...
			do {
				count++;
				E4D_LOGE("[ext %d]:\tstart %llu:\tlogical "
						"%llu:\tlen %llu\n", count,
						ext_list_tmp->data.physical,
						ext_list_tmp->data.logical,
	...
	if (mode_flag & DETAIL) {
		...
		if (current_uid == ROOT_UID) {
			if (strlen(msg_buffer) > 40)
				E4D_LOGE("\033[79;0H\033[K%s\n"
						"%50d/%-10d%6llu KB\n",
						msg_buffer, now_ext_count,
						best_ext_count, size_per_ext);
			else
				E4D_LOGE("\033[79;0H\033[K%-40s"
						"%10d/%-10d%6llu KB\n",
						msg_buffer, now_ext_count,
						best_ext_count, size_per_ext);
		} else {
			if (strlen(msg_buffer) > 40)
				...
				E4D_LOGE("\033[79;0H\033[K%-40s%10d/%-10s%7s\n",
						msg_buffer, now_ext_count,
							"-", "-");
		}
	}
...
static void print_progress(const char *file, loff_t start, loff_t file_size)
{
	...
	E4D_LOGE("\033[79;0H\033[K[%u/%u]%s:\t%3d%%",
	...
	do {
		...
		if (defraged_ret < 0) {
			if (mode_flag & DETAIL) {
				...
				if (errno == ENOTTY)
					E4D_LOGE("\tAt least 2.6.31-rc1 of "
			...
			struct FTW *ftwbuf EXT2FS_ATTR((unused)))
{
	...
	float ratio = 0.0;
	float score = 0.0;
	float defragment_score = 0.0;
	__u64 size_per_ext = 0;

	defraged_file_count++;
	E4D_LOGE("%s,%d defraged_file_count=%u\n",__func__,__LINE__, defraged_file_count);
	...
	if (buf->st_size < E4DEFRAG_MIN_SIZE || buf->st_size > E4DEFRAG_MAX_SIZE) {
		if (mode_flag & DETAIL) {
			E4D_LOGE("File size is %ld.\n", buf->st_size);
			PRINT_FILE_NAME(file);
			IN_FTW_PRINT_ERR_MSG("File size is needn't defragment.\n");
		}
		return 0;
	}
	...
	ratio = (float)(orig_physical_cnt - best) * 100 /
						blk_count;
	...
	E4D_LOGE("\n Total/best extents\t\t\t\t%d/%d\n"
		...
		" Fragmentation score\t\t\t\t%.0f\n",
			...
			best,
			size_per_ext, score);
	E4D_LOGE(" [0-30 no problem:"
		" 31-55 a little bit fragmented:"
		" 56- needs defrag]\n");

         if ((unsigned int)score <= E4DEFRAG_MIN_SCORE)
         {
                E4D_LOGE("score: %u, Needn't Defragment the file.\n", (unsigned int)score);
                goto out;
         }

         move_block_count += blk_count;
         E4D_LOGE("move_block_count: %llu.\n", move_block_count);
...
check_improvement:
	...
	ratio = (float)(donor_physical_cnt - best) * 100 / blk_count;
	defragment_score = CALC_SCORE(ratio);
	E4D_LOGE(" \n After Fragmentation score\t\t\t\t%.0f\n", defragment_score);

	/* file, score, defragment_score, blk_count, move_block_count */
	e4defrag_dmd_report(file, score, defragment_score, blk_count, move_block_count);
...
int fsinfo_get(void)
{
	...
	rtn = statvfs("/data", &s);
	if (rtn < 0) {
		E4D_LOGE("fsinfo_get statvfs failed.\n");
		return rtn;
	}

	E4D_LOGE("fsinfo_get: f_bsize: %lu, f_frsize: %lu, f_blocks: %lu, f_bfree: %lu, f_bavail: %lu.\n", s.f_bsize, s.f_frsize, s.f_blocks, s.f_bfree, s.f_bavail);

#if 0
	E4D_LOGE("fsinfo_get: f_bsize: %llu, f_frsize: %llu, f_blocks: %llu, f_bfree: %llu, f_bavail: %llu, 
		f_files: %llu, f_ffree: %llu, f_favail: %llu, f_fsid: %llu, f_flag: %llu, f_namemax: %llu.\n",
	s.f_bsize, s.f_frsize, s.f_blocks, s.f_bfree, s.f_bavail, s.f_files, s.f_ffree, s.f_favail, s.f_fsid, s.f_flag, s.f_namemax);
#endif
	return rtn;
}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "dvc")) != EOF) {
		switch (opt) {
		case 'd':
		case 'v':
			mode_flag |= DETAIL;
	...
	for (i = optind; i < argc; i++) {
		...
		E4D_LOGE("%s,%d Endian's type is not big/little endian\n",__func__,__LINE__);
		...
		if (lstat64(argv[i], &buf) < 0) {
			...
			PRINT_FILE_NAME(argv[i]);
			E4D_LOGE("%s,%d Failed to get file information\n",__func__,__LINE__);
			continue;
		}
		...
		if (S_ISBLK(buf.st_mode)) {
			...
			if (!(mode_flag & STATISTIC))
				E4D_LOGE("ext4 defragmentation for device(%s)\n",
					argv[i]);
		} else if (S_ISDIR(buf.st_mode)) {
			/* Directory */
			if (access(argv[i], R_OK) < 0) {
				perror(argv[i]);
				E4D_LOGE("%s,%d Failed to Access DIR.\n",__func__,__LINE__);
				continue;
			}
			arg_type = DIRNAME;
		...
		if (is_ext4(argv[i], dev_name) < 0) {
				E4D_LOGE("%s,%d is not ext4 filesystem.\n",__func__,__LINE__);
				continue;
		}
		if (realpath(argv[i], dir_name) == NULL) {
				perror("Couldn't get full path");
				PRINT_FILE_NAME(argv[i]);
				E4D_LOGE("%s,%d Couldn't get full path.\n",__func__,__LINE__);
				continue;
			}
		}
		...
		if (current_uid == ROOT_UID) {
			...
			if (ret) {
				...
				E4D_LOGE("%s,%d Couldn't get super block info.\n",__func__,__LINE__);
				continue;
			}
		...
		case DEVNAME:
			...
			E4D_LOGE("\n\tSuccess:\t\t\t[ %u/%u ]\n", succeed_cnt,
				total_count);
			E4D_LOGE("\tFailure:\t\t\t[ %u/%u ]\n",
				total_count - succeed_cnt, total_count);
			if (mode_flag & DETAIL) {
				E4D_LOGE("\tTotal extents:\t\t\t%4d->%d\n",
					extents_before_defrag,
					extents_after_defrag);
				E4D_LOGE("\tFragmented percentage:\t\t"
					"%3llu%%->%llu%%\n",
					!regular_count ? 0 :
					((unsigned long long)
		...
		case FILENAME:
			...
			if (mode_flag & STATISTIC) {
				...
				E4D_LOGE("ext4 defragmentation for %s\n",
								 argv[i]);
			/* Defrag single file process */
			file_defrag(argv[i], &buf, FTW_F, NULL);
			if (succeed_cnt != 0)
				E4D_LOGE(" Success:\t\t\t[1/1]\n");
			else
				E4D_LOGE(" Success:\t\t\t[0/1]\n");
		...
		if (mode_flag & STATISTIC) {
			...
			if (!succeed_cnt) {
				...
				if (arg_type == DEVNAME)
					E4D_LOGE(" In this device(%s), "
					"none can be defragmented.\n", argv[i]);
				else if (arg_type == DIRNAME)
					E4D_LOGE(" In this directory(%s), "
					"none can be defragmented.\n", argv[i]);
				else
					E4D_LOGE(" This file(%s) "
				...
				if (arg_type == DEVNAME)
					E4D_LOGE(" This device (%s) ", argv[i]);
				else if (arg_type == DIRNAME)
					E4D_LOGE(" This directory (%s) ",
								argv[i]);
				else
					E4D_LOGE(" This file (%s) ", argv[i]);

				if (score > BOUND_SCORE)
					E4D_LOGE("needs defragmentation.\n");
				else
					E4D_LOGE("does not need "
							"defragmentation.\n");
			}
			E4D_LOGE(" Done.\n");
		}
